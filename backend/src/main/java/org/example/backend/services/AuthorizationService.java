...
public class AuthorizationService implements OAuth2AuthorizationService {
    ...
    private final JsonMapper mapper = this.createSecurityJsonMapper();
    ...
    private Map<String, Object> parseMap(String data) {
        try {
            return this.mapper.readValue(data, MAP_TYPE_REFERENCE);
        } catch (Exception ex) {
    ...
    private JsonMapper createSecurityJsonMapper() {
        ...
        BasicPolymorphicTypeValidator.Builder validator = BasicPolymorphicTypeValidator.builder().allowIfSubType(AuthenticatedUserAuthenticationToken.class).allowIfSubType(UserPrincipal.class).allowIfSubType(ResourceBasedGrantedAuthority.class);

        return JsonMapper.builder().addModules(SecurityJacksonModules.getModules(loader, validator)).build();
    }
}
